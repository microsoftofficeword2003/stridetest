...
void PacBioCorrectionProcess::initCorrect(std::string& readSeq, std::vector<SeedFeature>& seedVec, std::vector<SeedFeature>& pacbioCorrectedStrs, PacBioCorrectionResult& result)
{
	...
	{				
		...
		for(int nextTargetSeed = 0 ; nextTargetSeed < (int)m_params.numOfNextTarget && targetSeed + nextTargetSeed < seedVec.size() ; nextTargetSeed++)
		{
			...
			cout<<srcKmerSize<<"srcKmerSize1 \n";
			std::string currseq = readSeq.substr(seedVec.at(targetSeed-1).seedEndPos-srcKmerSize+1) ;
			FMWalkReturnType = extendBetweenSeeds(source, target, mergedseq, extendKmerSize, dis_between_src_target,currseq );
			// cout<<FMWalkReturnType<<"<====FMWalkReturnType\n";
...
void PacBioCorrectionProcess::realCorrect(std::string& readSeq, std::vector<SeedFeature>& seedVec, std::vector<SeedFeature>& pacbioCorrectedStrs, PacBioCorrectionResult& result)
{
	...
	{				
		...
		for(int nextTargetSeed = 0 ; nextTargetSeed < (int)m_params.numOfNextTarget && targetSeed + nextTargetSeed < seedVec.size() ; nextTargetSeed++)
		{
			...
			std::string mergedseq;
			std::string currseq = readSeq.substr(source.seedEndPos-extendKmerSize-1,target.seedEndPos-source.seedEndPos+extendKmerSize+1) ;
			FMWalkReturnType = extendBetweenSeeds(source, target, mergedseq, extendKmerSize, dis_between_src_target,currseq);
...
int PacBioCorrectionProcess::checkseedcorrect(std::vector<SeedFeature> seeds,std::string currseedStr,size_t currseedStartpos)
{
	...
	size_t extendlength = currseedStartpos-seeds.at(seeds.size()-runtime).seedStartPos+1+seeds.at(seeds.size()-runtime).seedStr.length();
	 ...
	 int correctThreshold = 2;
	// if(seeds.size()>=2&&seeds.at(seeds.size()-1).isRepeat&&seeds.at(seeds.size()-2).isRepeat)
		// correctThreshold = 6;
...
std::vector<SeedFeature> PacBioCorrectionProcess::seedingByFixedKmer(const std::string readSeq, size_t contaminatedCutoff)
{
	...
	ofstream outfile ("seedfile.out") ;
 // ofstream kmerfile ("kmerfile.out") ;
	...
	for(size_t i = 0 ; i+kmerSize <= readSeq.length() ; i++)
	{
		...
		{	
			...
			if(maxKmerFreq > 17)	//40 should be determined using kmerDistribution
			{
			...
			// std::cout << "Seed:\t"<< seedVec.back().seedLength << "\t" << seedVec.back().seedStr << "\t"<<maxKmerFreq <<"\n";
		      // outfile<< seedVec.back().seedStr << "\n";
			...
			// cout << seedVec.back().seedStr << "===\n";
			// cout << "0.0" ;
	}// end of for
	for(size_t j=0;j<seedVec.size();j++)
	{
		
		      outfile<< seedVec.at(j).seedStr <<"\n"<<seedVec.at(j).isRepeat<< "\n";
	}
	outfile.close();
	// kmerfile.close();
	return seedVec;
...
int PacBioCorrectionProcess::extendBetweenSeeds(SeedFeature& source, SeedFeature& target, std::string& mergedseq, 
												size_t extendKmerSize, size_t dis_between_src_target,std::string currseq)
{	
	...
	cout<< FMWalkReturnType << "\t i'm here \n";
